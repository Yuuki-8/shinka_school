...
namespace :notification do
  ...
  task event_before_one_hour_to_slack: :environment do
    puts "notification:event_before_one_hour_to_slack... start"
    events = Event.where("deadline_date >= ? and deadline_date <= ?", Time.zone.now, Time.zone.now + 1.hour)
    puts "対象件数：#{events.count}件"
    if events.present?
      events.map do |event|
        unless event.is_before_one_hour_notification
          event.notification_event_before_one_hour_to_slack
          event.is_before_one_hour_notification = true
          event.save!
        end
      end
    end
    puts "notification:event_before_one_hour_to_slack... end"
  end
  desc "イベントの締め切り時刻に通知"
    ...
    if events.present?
      events.map do |event|
        unless event.is_just_notification
          event.notification_event_just_to_slack
